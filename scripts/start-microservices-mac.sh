...
wait_for_port() {
  ...
  while true; do
    ...
    if [[ -n "$pid" ]] && ! kill -0 "$pid" >/dev/null 2>&1; then
      echo "[error] $name finalizo antes de abrir $host:$port"
      if [[ -n "$log_file" && -f "$log_file" ]]; then
        echo "[error] Ultimas lineas de $log_file:"
        tail -n 20 "$log_file"
      fi
      return 1
    fi
...
start_service() {
  ...
  local extra_args="${4:-}"
  local service_path="$ROOT_DIR/$service_dir"
  local log_file="$LOG_DIR/$service_name.log"
  local -a mvn_cmd

  echo "[start] Iniciando $service_name..."

  if [[ -x "$service_path/mvnw" ]]; then
    mvn_cmd=("$service_path/mvnw")
  elif [[ -f "$service_path/mvnw" ]]; then
    mvn_cmd=("bash" "$service_path/mvnw")
  elif command -v mvn >/dev/null 2>&1; then
    mvn_cmd=("mvn")
  else
    echo "[error] No se encontro mvnw ni mvn para $service_name"
    return 1
  fi

  (
    cd "$service_path"
    "${mvn_cmd[@]}" $MVN_COMMON_ARGS spring-boot:run $extra_args > "$log_file" 2>&1
  ) &
  ...
  wait_for_port "localhost" "$service_port" "$service_name" "120" "$pid" "$log_file"
}
